...
class CostTests: XCTestCase {
    ...
    func testMatches() {
        let cost1 = Cost(amount: amount1, date: nil, label: label1)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
    ...
    func testMatchesLabel() {
        let cost1 = Cost(amount: nil, date: nil, label: label1)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertTrue(cost1.matches(cost: cost2))
    }

    func testMatchesDate() {
        let cost1 = Cost(amount: nil, date: date1, label: nil)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertTrue(cost1.matches(cost: cost2))
    }

    func testMatchesAmount() {
        let cost1 = Cost(amount: amount1, date: nil, label: nil)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertTrue(cost1.matches(cost: cost2))
    }

    func testNotMatchesLabel() {
        let cost1 = Cost(amount: nil, date: nil, label: label2)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertFalse(cost1.matches(cost: cost2))
    }

    func testNotMatchesDate() {
        let cost1 = Cost(amount: nil, date: date2, label: nil)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertFalse(cost1.matches(cost: cost2))
    }

    func testNotMatchesAmount() {
        let cost1 = Cost(amount: amount2, date: nil, label: nil)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertFalse(cost1.matches(cost: cost2))
    }

    func testMatchesLabelWrong() {
        let cost1 = Cost(amount: amount1, date: date1, label: label2)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertFalse(cost1.matches(cost: cost2))
    }

    func testMatchesDateWrong() {
        let cost1 = Cost(amount: amount1, date: date2, label: label1)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertFalse(cost1.matches(cost: cost2))
    }

    func testMatchesAmountWrong() {
        let cost1 = Cost(amount: amount2, date: date1, label: label1)
        let cost2 = Cost(amount: amount1, date: date1, label: label1)
        XCTAssertFalse(cost1.matches(cost: cost2))
    }
